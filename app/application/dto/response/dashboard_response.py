...
class SubjectSummaryDto(BaseModel):
    ...
    current_average: float = Field(
        ..., description="Promedio actual en escala 0.0 – 5.0"
    )
...
class TaskSummaryDto(BaseModel):
    ...
    completion_rate: float = Field(
        ..., description="Tasa de completitud en porcentaje (0.0 – 100.0)"
    )


class DashboardResponseDto(BaseModel):
    user_id: str
    overall_gpa: float = Field(
        ..., description="Promedio académico ponderado (0.0 – 5.0)"
    )
